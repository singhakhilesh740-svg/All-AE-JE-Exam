...
export async function loginWithEmail(email, password) {
  ...
  const user = cred.user;
  // If Firebase Auth login succeeds but Firestore profile is missing,
  // auto-create profile instead of blocking (handles password-reset users)
  const profile = await getUserProfile(user.uid);
  if (!profile) {
...
export async function loginWithGoogle() {
  ...
  if (!profile) {
    ...
    if (existingUid) {
      // Email registered under different UID — copy profile to Google UID
      ...
      // No profile anywhere — auto-create from Google account data
      // (handles users who signed up via Google before this system)
      await saveUserProfile({
        uid:    user.uid,
        name:   user.displayName || 'Student',
        email:  user.email,
